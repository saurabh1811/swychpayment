    ...
    <UiTitleCard title="PUSD Transactions" class-name="px-0 rounded-md overflow-hidden">
            ...
            <template v-else>
                <DataTable ref="dataTable" v-model:filters="filters" v-model:selection="selectedCustomers" :value="customers" paginator :rows="10" dataKey="id" filterDisplay="menu"
                    ...
                    <Column header="Status" field="data.attributes.status" sortable :filterMenuStyle="{ width: '14rem' }" style="min-width: 12rem">
                        <template #body="{ data }">
...
<script setup>
        ...
        const statuses = ref(['Processed', 'Pending', 'Rejected']);
        const types = ref([
        ...
        const initFilters = () => {
            filters.value = {
                ...
                'attributes.status': { operator: FilterOperator.OR, constraints: [{ value: null, matchMode: FilterMatchMode.EQUALS }] },
